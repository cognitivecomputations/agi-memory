-- Refresh the stale prompt_modules copy of rlm_heartbeat_system (#115):
-- the *file* (services/prompts/rlm_heartbeat_system.md, what the RLM path
-- actually loads at runtime -- see load_rlm_heartbeat_prompt()) was fixed
-- in c08c8f5 to stop telling the model `reflect` is a tool_use target, and
-- the baseline seed (db/40) picked that fix up for fresh installs, but no
-- migration ever re-upserted the catalog row itself -- so any database that
-- was already migrated past that point still carries the old text. Nothing
-- reads this row at runtime (the Python loader reads the file directly), so
-- this is a pure catalog-drift fix, harmless either way, matching the same
-- upsert-only staleness class this issue calls out for tool_definitions.
